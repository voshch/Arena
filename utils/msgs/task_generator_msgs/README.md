...
| `DespawnObstacle.srv` | Remove one obstacle added by `SpawnStatic` / `SpawnDynamic`, via `TM_Obstacles.retract`. `id` is what the spawn returned (the entity's `sim_path`), and a plain name resolves too. `found` is reported separately from `success` because an id that resolved to nothing is the state the caller wanted, not a failure. Removing one *pedestrian* needs a backend that keeps a per-agent id map, which is `arena_humansim` only; elsewhere the id comes back not-removed rather than clearing the whole crowd. |
