...
_INFERENCE_RETRY_INTERVAL = float(os.getenv("VLA_INFERENCE_RETRY_INTERVAL", 5.0))
...
class TM_VLA(TM_Robots):
    ...
    async def reset(self, **kwargs: object) -> None:
        ...
        try:
            ...
            self.node.get_logger().error(f"[TM_VLA]: VLA Server not reachable at {_VLA_BASE}")
        ...
        for name, robot in self._ctx.robots.items():
            ...
            if topic is None:
                self.node.get_logger().warn(f"[TM_VLA:{name}] no image sensor in model_params. Skipping {name}")
                continue
            sub = self.node.create_subscription(sensor_msgs.msg.Image, topic, lambda msg, n=name: self.robot_image(n, msg), qos)
            self._image_subs.append(sub)
            self.node.get_logger().info(f"[TM_VLA]sub to {topic}")
    ...
    async def done(self) -> bool:
        ...
        for name, robot in self._ctx.robots.items():
            if self._near_goal.get(name, False):
                self.node.get_logger().warn(f"[TM_VLA:{name}] proximity mode — waiting for nav2")
    ...
    # ---------------------help func--------------
    def find_image_topic(self, robot: Robot) -> str | None:
    ...
    # -----------------------
    def robot_image(self, robot: str, msg: sensor_msgs.msg.Image) -> None:
    ...
    def vla_inference(self, image: sensor_msgs.msg.Image) -> list[tuple[float, float]] | None:
        ...
        try:
            response = requests.post(_VLA_SERVER, files={"image": ("frame.jpg", buf.getvalue(), "image/jpeg")}, data={"instruction": _INSTRUCTION}, timeout=10.0)
    ...
    # ----------------------
    async def submit_vla_goal(self, name: str, robot: Robot) -> None:
        # Do the following:
        ...
        if image is None or current_pose is None:
            self.node.get_logger().warn("No image or pose(most likely img)")
            return
        ...
        if waypoints is None:
            self.node.get_logger().warn("waypoint empty")
            return
        ...
        if not phases:  # incase all invalide
            ...
            self.node.get_logger().warn(f"[TM_VLA:{name}] all waypoints invalid ({streak}/{_MAX_INVALID_STREAK}) — skipping")
            if streak >= _MAX_INVALID_STREAK:
                self.node.get_logger().warn(f"[TM_VLA:{name}] invalid streak limit reached — forcing proximity handoff")
        ...
        total_dist = sum(math.hypot(wps[i + 1].position.x - wps[i].position.x, wps[i + 1].position.y - wps[i].position.y) for i in range(len(wps) - 1))
        self._near_goal[name] = total_dist < _WAYPOINT_PROXIMITY_THRESHOLD
        if self._near_goal[name]:
            phases = [phases[-1]]
            self.node.get_logger().warn(f"[TM_VLA:{name}] proximity mode — total_dist={total_dist:.2f}m < {_WAYPOINT_PROXIMITY_THRESHOLD}m, handing off final goal to nav2")
