...
def _solve_wire_triple(collar_r: np.ndarray, u: np.ndarray, f: np.ndarray, bend: float, warm: tuple[float, float, float] = (0.0, 0.0, 0.0)) -> tuple[float, float, float]:
    ...
    def score(sol: tuple[float, float, float]) -> float:
        yy, pp, rr = sol
        pen = C.saturation("l_y_shoulder", yy) + C.saturation("l_p_shoulder", pp) + C.saturation("l_r_shoulder", rr)
        drift = abs(C.wrap_pi(yy - warm[0])) + abs(pp - warm[1]) + abs(C.wrap_pi(rr - warm[2]))
...
def solve_arm_ik(
    ...
    for phi in phis:
        ...
        cost = aim_err + opts.limit_weight * sat + opts.collide_weight * max(0.0, -clear) + opts.swivel_prior * (1.0 - np.cos(phi - swivel_pref))
        cand = {
            "cost": float(cost),
            "phi": float(phi),
            "raw": raw,
            "angles": {
                f"{side}_y_shoulder": yc,
                f"{side}_p_shoulder": pc,
                f"{side}_r_shoulder": rc,
                f"{side}_elbow": bc,
            },
...
class PointAtGenerator:
    ...
    def __init__(self, template: str = "point_to_right", height: float = 1.65, options: PointAtOptions | None = None):
    ...
    def point_at(
        ...
        **kw: object,
        ...
        r_src_anchor = fr_a_src[f"{side}_collar"] @ S.arm_rot(*(self._arm_track(anchor, side)[n] for n in tri))

        def delta_for(posed: dict) -> np.ndarray:
            _, fr_a_out = S.fk(posed, self.body)
            return (fr_a_out[f"{side}_collar"] @ S.arm_rot(*(posed[n] for n in tri))) @ r_src_anchor.T
        ...
        if path:
            ...
            cands = solve_arm_ik(pos_a, fr_a, side, anchor_sol["aim"], self.body, bend, self._swivel_pref(side), opts, warm=warm, all_candidates=True)
            floor = cands[0]["cost"]
        ...
        for k, (i, w) in enumerate(zip(idx, w_track, strict=True)):
            ...
            prev_tri = None if not frames_out else tuple(frames_out[-1]["angles"][n] for n in tri)
            ...
            r_world = S.slerp_rot(np.eye(3), r_delta, float(w)) @ (fr_src[f"{side}_collar"] @ S.arm_rot(*(src_ang[n] for n in tri)))
            if steady[k] > 0.0:
                r_solved, warm = self._steady_solve(pos_out, fr_out, side, aim0, target_point, out[f"{side}_elbow"], warm, opts, phi)
                r_world = S.slerp_rot(r_world, r_solved, float(steady[k]))
            r_out = fr_out[f"{side}_collar"].T @ r_world
            for n, v in zip(tri, _triple_in_limits(side, S.arm_rot_inverse(r_out), prev=prev_tri, limits=emit), strict=True):
                out[n] = v
            ...
            diag.append({"w": float(w), "sol": anchor_sol, "src": i, "steady": float(steady[k]), "anchor": bool(i == anchor)})
        ...
        if opts.relax_on_collision and opts.elbow_bend is None and clip.report["clearance_m"] < 0.0:
            best = clip
            for extra in (0.22, 0.45, 0.70):
                alt = self.point_at(
                    ...
                    deg=deg,
                    options=opts,
                    elbow_bend=bend + extra,
                    relax_on_collision=False,
                )
                # clearance is worth buying, but not with the aim
                if alt.report["clearance_m"] > best.report["clearance_m"] and alt.report["aim_error_deg"] <= max(1.0, clip.report["aim_error_deg"]):
                    best = alt
    ...
    # -- helpers ----------------------------------------------------------
    def _steady_solve(self, pos_out: dict, fr_out: dict, side: str, aim0: np.ndarray, target_point: np.ndarray | None, bend_i: float, warm: tuple[float, float, float], opts: PointAtOptions, phi: float | None, swivel_pref: float | None = None) -> tuple[np.ndarray, tuple[float, float, float]]:
        ...
        if target_point is not None:
            aim_i = self._aim_for_point(pos_out, fr_out, side, target_point, bend_i, warm, opts, phi, swivel_pref=pref)
        sol_i = solve_arm_ik(pos_out, fr_out, side, aim_i, self.body, bend_i, pref, opts, warm=warm, fixed_phi=phi)
        warm = tuple(sol_i["angles"][n] for n in tri)
        return fr_out[f"{side}_collar"] @ S.arm_rot(*(sol_i["angles"][n] for n in tri)), warm

    def _resolve_aim(self, target: Sequence[float] | np.ndarray | None, azimuth: float | None, elevation: float | None, deg: bool) -> tuple[np.ndarray, np.ndarray | None]:
    ...
    def _near_target(self, target_point: np.ndarray, side: str) -> bool:
        ...
        return float(np.linalg.norm(target_point - pos_pk[f"{side}_shoulder"])) <= (self.body.arm_reach + 0.45)
    ...
    def _frame_deltas(self, i: int, aim0: np.ndarray, side: str, d_yaw: float, d_pitch: float, bend: float, opts: PointAtOptions) -> dict[str, float]:
    ...
    def _solve_frame(self, i: int, aim0: np.ndarray, side: str, d_yaw: float, d_pitch: float, bend: float, target_point: np.ndarray | None, opts: PointAtOptions, warm: tuple[float, float, float], fixed_phi: float | None, swivel_pref: float | None = None) -> tuple[dict, dict, dict[str, float]]:
        ...
        sol = solve_arm_ik(pos, fr, side, aim, self.body, bend, pref, opts, warm=warm, fixed_phi=fixed_phi)
    ...
    def _blended_dofs(self, side: str) -> list[str]:
        return [f"y_{s}" for s in C.SPINE_SEGMENTS] + list(C.SPINE_SEGMENTS) + [f"r_{s}" for s in C.SPINE_SEGMENTS] + C.arm_dofs(side)
    ...
    def _plateau(self, want: int) -> list[int]:
        ...
        return [h0 + (j % period if (j % period) < span else period - (j % period)) for j in range(want)]

    def _aim_for_point(self, pos: dict, fr: dict, side: str, target_point: np.ndarray, bend: float, warm: tuple[float, float, float], opts: PointAtOptions, fixed_phi: float | None = None, swivel_pref: float | None = None) -> np.ndarray:
    ...
    def analyze(self, clip: PointAtClip, diag: list | None = None) -> dict:
        ...
        side = clip.side
        s0, s1 = self.template.settled
        settled = [i for i, f in enumerate(clip.frames)] if diag is None else [i for i, d in enumerate(diag) if s0 <= d["src"] < s1]
        ...
        hold = hold or settled or ([int(np.argmax([d["w"] for d in diag]))] if diag else [0])
        anchor_i = next((i for i, d in enumerate(diag or []) if d.get("anchor")), hold[len(hold) // 2])
        rows = []
        for i in hold:
            ...
            if diag is not None:
                ...
                sat_joints = {k: round(float(v), 4) for k, v in ((k, C.saturation(k, v)) for k, v in diag[i]["sol"]["raw"].items()) if v > 1e-4}
        ...
        for i in settled:
            ...
            want_i = S.unit(clip.target_point - pos_i[f"{side}_wrist"]) if clip.target_point is not None else clip.target_dir
            settle_err = max(
                settle_err,
        ...
        return {
            ...
            "saturated_joints": {k2: v2 for r in rows for k2, v2 in r[5].items()},
            "ok": bool(errs.max() < 5.0 and clear_v > -0.005),
        }


def point_at(target: Sequence[float] | np.ndarray | None = None, azimuth: float | None = None, elevation: float | None = None, template: str = "point_to_right", height: float = 1.65, **kw: object) -> PointAtClip:
    """One-shot convenience wrapper around :class:`PointAtGenerator`."""
    return PointAtGenerator(template=template, height=height).point_at(target=target, azimuth=azimuth, elevation=elevation, **kw)
