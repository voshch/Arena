...
def fk(angles: dict, body: Body, root_xy_yaw: tuple[float, float, float] = (0.0, 0.0, 0.0)) -> tuple[dict, dict]:
    ...
    for side in ("l", "r"):
        ...
        s_r = c_r @ arm_rot(a[f"{side}_y_shoulder"], a[f"{side}_p_shoulder"], a[f"{side}_r_shoulder"])
    ...
    for side in ("l", "r"):
        ...
        h = root_r @ rot_axis(x1, a[f"{side}_y_hip"]) @ rot_axis(x2, a[f"{side}_p_hip"]) @ rot_axis(x3, a[f"{side}_r_hip"])
...
def collision_clearances(pos: dict, body: Body, side: str) -> dict[str, float]:
    ...
    for tag, seg in (("upperarm", upper_trim), ("forearm", fore)):
        ...
        out[f"{tag}/neck"] = _segment_distance(*seg, pos["torso"], pos["neck"]) - (ra + 0.6 * body.r_torso)
        for oth_tag, oth in (
