...
class PointGesture:
    ...
    def _aim(self, local: np.ndarray) -> tuple[np.ndarray, dict]:
        """Aim direction for ``local`` and the generator kwargs that reproduce it."""
        return self._gen.aim_of(local), {"target": local}
    ...
    def resolve_hand(self, local: np.ndarray, opts: dict) -> str:
        ...
        return self._gen.choose_hand(self._aim(local)[0], self._options({**opts, "hand": "auto"}))
    ...
    def bind(self, slot: str, local: np.ndarray, opts: dict) -> dict:
        ...
        hand = self.SLOT_HAND.get(slot) or self.resolve_hand(local, opts)
        return {**opts, "hand": hand}

    def start(self, local: np.ndarray, opts: dict) -> GestureClip:
        clip = self._gen.point_at(**self._aim(local)[1], options=self._options(opts))
    ...
    def retarget(self, hold: object, local: np.ndarray, opts: dict) -> GestureClip:
        ...
        aim, at = self._aim(local)
        swept = math.acos(float(np.clip(np.dot(S.unit(hold.target_dir), aim), -1.0, 1.0)))
        clip = self._gen.retarget(hold, **at, options=options, transition_s=move_time(swept))
