...
def test_hold_pose_round_trip(gen):
    ...
    again = gen.hold_pose(gen.retarget(hp, azimuth=-10, elevation=-10))
    assert again.side == hp.side and again.target_point is None


@pytest.mark.parametrize("start,new", [((-40, 25), (-10, -10)), ((-40, 25), (-70, 40)), ((60, 20), (40, -60)), ((-20, 0), (-5, 80))])
def test_retarget_continuity(gen, start, new):
    prev = gen.hold_pose(gen.point_at(azimuth=start[0], elevation=start[1]))
    clip = gen.retarget(prev, azimuth=new[0], elevation=new[1])
    first = clip.frames[0]["angles"]
    assert max(abs(first[n] - prev.angles[n]) for n in prev.angles) < 1e-6
    want = direction_from_angles(*new)
    last = aim_of(clip, gen.body, len(clip.frames) - 1)
    assert np.degrees(np.arccos(np.clip(float(last @ want), -1, 1))) < 2.0
    assert clip.report["aim_error_deg"] < 2.0
    assert clip.report["max_joint_step_rad"] < 1.0
    assert float(np.abs(np.diff(joint_matrix(clip), axis=0)).max()) < 1.0
    assert clip.side == prev.side
    a, b = clip.hold_range
    assert a == clip.report["transition_frames"] and b == len(clip.frames) - 1
    assert {"aim_error_deg", "clearance_m", "max_joint_step_rad"} <= set(clip.report)


def test_retarget_point_target(gen):
    prev = gen.hold_pose(gen.point_at(target=(2.5, -1.0, 1.6)))
    clip = gen.retarget(prev, target=(3.0, -0.5, 0.5), transition_s=0.2)
    assert clip.report["transition_frames"] == 4
    assert clip.report["aim_error_deg"] < 1.0
    assert clip.report["max_joint_step_rad"] < 1.0
    with_hold = gen.retarget(prev, target=(3.0, -0.5, 0.5), options=PointAtOptions(hold_s=2.0))
    assert len(with_hold.frames) > len(clip.frames)


def test_retarget_hand_switch(gen):
    prev = gen.hold_pose(gen.point_at(azimuth=-40, elevation=25))
    assert prev.side == "r"
    with pytest.raises(ValueError, match="hand switch"):
        gen.retarget(prev, azimuth=60, elevation=0)
    forced = gen.retarget(prev, azimuth=60, elevation=0, options=PointAtOptions(hand="r"))
    assert forced.side == "r"


@pytest.mark.parametrize("start,new", [((-40, 25), (-30, 20)), ((60, 20), (55, 35)), ((-20, 0), (-25, -15))])
def test_retarget_light_continuity_and_aim(gen, start, new):
    prev = gen.hold_pose(gen.point_at(azimuth=start[0], elevation=start[1]))
    clip = gen.retarget_light(prev, azimuth=new[0], elevation=new[1], transition_s=0.3)
    first = clip.frames[0]["angles"]
    assert max(abs(first[n] - prev.angles[n]) for n in prev.angles) < 1e-6
    want = direction_from_angles(*new)
    last = aim_of(clip, gen.body, len(clip.frames) - 1)
    assert np.degrees(np.arccos(np.clip(float(last @ want), -1, 1))) < 2.0
    assert clip.report["aim_error_deg"] < 2.0
    assert clip.report["swivel_rad"] == prev.swivel
    assert clip.report["light"] is True
    assert clip.report["transition_frames"] == 6
    assert clip.report["max_joint_step_rad"] < 1.0
    assert clip.side == prev.side
    a, b = clip.hold_range
    assert a == 6 and b == len(clip.frames) - 1


def test_retarget_light_hand_switch(gen):
    prev = gen.hold_pose(gen.point_at(azimuth=-40, elevation=25))
    with pytest.raises(ValueError, match="hand switch"):
        gen.retarget_light(prev, azimuth=60, elevation=0)
