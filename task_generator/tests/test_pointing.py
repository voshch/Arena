...
def test_segment_distance_vs_brute_force():
    ...
    for _ in range(200):
        ...
        brute = min(np.linalg.norm((p0 + t * (p1 - p0)) - (q0 + u * (q1 - q0))) for t in ts for u in ts[::8])
...
def test_template(name):
    ...
    assert all(abs(t.base[i][d] + t.w[i] * (t.frames[t.peak]["angles"][d] - t.frames[0]["angles"][d]) - t.frames[i]["angles"][d]) < 1e-12 for i in (0, t.peak, len(t.frames) - 1) for d in C.arm_dofs(t.side))
...
def test_wire_contract(gen):
    ...
    assert all(C.saturation(n, v, limits=C.EMIT_LIMITS) == 0.0 for fr in clip.frames for n, v in fr["angles"].items() if n in written)
