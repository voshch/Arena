...
def _names() -> list[str]:
    ...
    return sorted({*all_robots(arena), *component_families(arena)})
...
def add(argv: list[str]) -> None:
    ap = argparse.ArgumentParser(prog="robots add")
    ap.add_argument("names", nargs="*", help="robot names and/or component refs (<type>/<name>), a robot also pulls the components its assembly mounts by default")
    ap.add_argument("--all", action="store_true", help="fetch every robot and component")
    args = ap.parse_args(argv)
    os.environ.setdefault("GIT_SSH_COMMAND", common._git_ssh_command())
    rc = cmd_add(Path(common._env("ARENA_DIR")), args)
...
def update(argv: list[str]) -> None:
    ...
    args = ap.parse_args(argv)
    os.environ.setdefault("GIT_SSH_COMMAND", common._git_ssh_command())
    rc = cmd_update(Path(common._env("ARENA_DIR")), args)
    complete.invalidate(common._env("ARENA_WS_DIR"))
    sys.exit(_deps_build() or rc)


def rm(argv: list[str]) -> None:
    ap = argparse.ArgumentParser(prog="robots rm")
    ap.add_argument("names", nargs="*", help="robot names and/or component refs (<type>/<name>)")
    ap.add_argument("--all", action="store_true", help="remove every robot and component")
    ap.add_argument("-f", "--force", action="store_true", help="deinit shared paths too, co-tagged robots become pending")
    args = ap.parse_args(argv)
    os.environ.setdefault("GIT_SSH_COMMAND", common._git_ssh_command())
    rc = cmd_rm(Path(common._env("ARENA_DIR")), args)
    complete.invalidate(common._env("ARENA_WS_DIR"))
    sys.exit(rc)


def ls(argv: list[str]) -> None:
    ap = argparse.ArgumentParser(prog="robots ls")
    args = ap.parse_args(argv)
    sys.exit(cmd_ls(Path(common._env("ARENA_DIR")), args))


def check(argv: list[str]) -> None:
    ap = argparse.ArgumentParser(prog="robots check")
    ap.add_argument("--all", action="store_true")
    ap.add_argument("-q", "--quiet", action="store_true")
    args = ap.parse_args(argv)
    sys.exit(cmd_check(Path(common._env("ARENA_DIR")), args))


def drive(argv: list[str]) -> None:
    ap = argparse.ArgumentParser(prog="robots drive")
    ap.add_argument("kv", nargs="*", help="bare tokens select robots (default: all ready), key:=value pairs for map/episodes/timeout are consumed locally, the rest forwards to the benchmark runner")
    args = ap.parse_args(argv)
    sys.exit(cmd_drive(Path(common._env("ARENA_DIR")), args))
...
def uninstall(argv: list[str]) -> None:
    if argv:
        rm(argv)
        return
    ap = argparse.ArgumentParser(prog="robots uninstall")
    args = ap.parse_args([])
    os.environ.setdefault("GIT_SSH_COMMAND", common._git_ssh_command())
